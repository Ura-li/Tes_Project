...
export function NmuItemEdit({ NmuItemId, onUpdate }) {
  const [open, setOpen] = React.useState(false)

  const nmuItemEditQuery = useQuery({
    queryKey: ["nmuItem", NmuItemId],
    enabled: open && !!NmuItemId,
    queryFn: async () => {
      const res = await ApiCustomer.get(`/api/nmu/nmuitem/${NmuItemId}`)
  ...
  const updateMutation = useMutation({
    mutationFn: async (values) => {
      ...
      await ApiCustomer.patch(`/api/nmu/nmuitem/${NmuItemId}`, payload)
    },
    onSuccess: () => {
      onUpdate?.()
      setOpen(false)
  ...
  React.useEffect(() => {
    if (!nmuItemEditQuery.data) return
    const data = nmuItemEditQuery.data
    form.reset({
        itemName: data?.itemName,
        nmu: data?.nmuId ? {NMUId: data.nmuId, NMUDesc: data.nmu.NMUDesc} : undefined,
    })
  }, [nmuItemEditQuery.data])
  ...
  return (
    <>
      <FormDialog
        ...
        title="Edit NMU Item Information"
      ...
      >
        ...
        {nmuItemEditQuery.isFetching ? <p className="text-sm text-muted-foreground mt-2">Loading nmu item...</p> : null}
      </FormDialog>
    </>
  )
}
