...
export function UserTemplateButton() {
  const handleDownload = () => {
    window.location.href = `${import.meta.env.VITE_API_BASE_URL}/api/import/user`;
  };
...
export function UserImport() {
    ...
    const handleImport = async () => {
        ...
        try{
            const response = await ApiCustomer.post("/api/import/user",formData);
    ...
    return (
        <div className="flex flex-row items-center gap-2 ">
            ...
            <Button variant={"outline"} onClick={handleImport}>Import User Data</Button>
        </div>
    );
}